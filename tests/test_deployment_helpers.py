...
class BuildRepeatabilityTests(unittest.TestCase):
    ...
    def test_build_script_uses_pinned_environment(self):
        wrapper = (REPO_ROOT / "build.bat").read_text(encoding="utf-8")
        build_script = (REPO_ROOT / "tools/Build-Release.ps1").read_text(encoding="utf-8")
        self.assertIn("tools\\Build-Release.ps1", wrapper)
        for command in ("-m venv .build-venv", "-m pip install --requirement requirements.txt", "-m pip check", "-m unittest discover", "create_icon.py", "-m PyInstaller --noconfirm --clean ExplorerTweaks.spec"):
            self.assertIn(command, build_script)
        self.assertIn("[IO.Path]::GetDirectoryName($path) -ne $repo", build_script)
        self.assertNotIn("rmdir /s", wrapper)

    def test_build_script_creates_release_trust_artifacts(self):
        build_script = (REPO_ROOT / "tools/Build-Release.ps1").read_text(encoding="utf-8")
        packager = (REPO_ROOT / "tools/package_release.py").read_text(encoding="utf-8")
        self.assertIn("Set-AuthenticodeSignature", build_script)
        self.assertIn("The executable is unsigned", build_script)
        self.assertIn("FileMajorPart", build_script)
        for filename in ("INSTALL.txt", "README.md", "LICENSE", "SHA256SUMS.txt", "release-manifest.json"):
            self.assertIn(filename, packager)
        self.assertIn("zipfile.ZipInfo", packager)
        self.assertIn("ZIP differs from verified package inputs", packager)
