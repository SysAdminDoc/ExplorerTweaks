#!/usr/bin/env python3
"""
ExplorerTweaks v2.16.2 - Windows File Explorer Configuration Utility
...
def get_windows_version() -> OSVersion:
    ...
    except Exception: return OSVersion.UNKNOWN
    ...
    for path, slist in by_path.items():
        ...
        for s in slist:
            ...
            if val is None:
                val = s.default_value
            if s.reg_type == "DWORD":
                lines.append(f'"{s.reg_name}"=dword:{int(val):08x}')
...
class TaskbarPreview(BasePreview):
    ...
    def _render_info(self):
        ...
        for label, val in [
            ...
            ("Search", ["Hidden", "Icon", "Box"][min(self.state.search_mode, 2)]),
...
class App(ctk.CTk):
    def __init__(self, initial_focus: Optional[str] = None):
        ...
        self.nav_buttons = {}
        self.previews = {}
        self._pre_search_cat: Optional[str] = None
    ...
    def _load_preview_state(self):
        ...
        self.preview_state.silent_installs = gb(cdm, "SilentInstalledAppsEnabled", 1, True)
        self.preview_state.bing_search = gb(search, "BingSearchEnabled", 1, True)
        self.preview_state.cortana = gb(search, "CortanaConsent", 1, True)
        self.preview_state.search_history = gb(search, "HistoryViewEnabled", 1, True)
        ...
        self.preview_state.end_task = gb(dev, "TaskbarEndTask", 1, False)
    ...
    def _on_search(self, *args):
        ...
        if not query:
            restore = self._pre_search_cat or (self.categories[0] if self.categories else None)
            if restore:
                self._pre_search_cat = None
                self._on_nav(restore)
            return
        if self.current_cat != "_search":
            self._pre_search_cat = self.current_cat
    ...
    def _show_diff(self, profile_name: str, profile_settings: dict):
        ...
        for sid, target_val in profile_settings.items():
            if sid in ("_classic", "classic_context_menu"):
                current = SpecialSettings.get_classic_context_menu()
                target = bool(target_val)
                dummy = type("S", (), {"name": "Classic Context Menu"})()
                if current != target:
                    diffs.append((dummy, current, target))
                else:
                    matches.append((dummy, current))
                continue
            if sid in ("_search", "search_mode"):
                current = SpecialSettings.get_search_mode()
                target = int(target_val)
                dummy = type("S", (), {"name": "Taskbar Search Mode"})()
                if current != target:
                    diffs.append((dummy, current, target))
                else:
                    matches.append((dummy, current))
    ...
    def _import(self):
        ...
        if path:
            try:
                ...
                messagebox.showerror("Import", f"Could not read profile:\n{exc}")
                return
